...
import OreFinder from './engines/ore-finder-component';
import ChunkBase from './engines/chunk-base-component';

export default function App() {
  // ── Tab State ──────────────────────────────────────────────────────────────
  const [activeTab, setActiveTab] = useState('bedrock');

  // ── Bedrock Finder State ───────────────────────────────────────────────────
  ...
  const calculateCoordinates = () => {
    ...
    const workerCode = edition === 'java' ? `
      ...
      function generateBedrock(worldSeed, x, z, y, dimension) {
        const bedrockPattern = [];
        const versionNum = ${versionNum};
        let minY, maxY, solidLayer;
        if (dimension === 'nether') {
          minY = 123; maxY = 127; solidLayer = 127;
        } else if (versionNum >= 1.18) {
          minY = -64; maxY = -59; solidLayer = -64;
        } else {
          minY = 0; maxY = 4; solidLayer = 0;
        }
        for (let dy = 0; dy <= (maxY - minY); dy++) {
          const currentY = y + dy;
          if (currentY === solidLayer) { bedrockPattern.push(true); continue; }
          const blockX = BigInt(x);
          const blockZ = BigInt(z);
          const blockY = BigInt(currentY);
          const wSeed = BigInt(worldSeed);
          const mixedSeed = wSeed * 341873128712n + blockX * 132897987541n;
      ...
      function matchesPattern(worldSeed, startX, startZ, pattern, gridSize, layer, dimension) {
        ...
        for (let row = 0; row < gridSize; row++) {
          for (let col = 0; col < gridSize; col++) {
            ...
            const bedrockLayers = generateBedrock(worldSeed, worldX, worldZ, baseY, dimension);
            const hasBedrock = bedrockLayers[0];
            if (pattern[row][col] !== hasBedrock) return false;
          }
        }
        return true;
      }
      ...
      self.onmessage = function(e) {
        ...
        while (!found && radius <= 25000) {
          for (let x = xStart; x <= xEnd; x += 1) {
            for (let z = zStart; z <= zEnd; z += 1) {
              ...
              if (tested % updateInterval === 0) {
                self.postMessage({ type: 'progress', workerId, x, z, tested, radius: radius * 4 });
              }
              if (matchesPattern(worldSeed, x, z, pattern, gridSize, layer, dimension)) {
                self.postMessage({ type: 'found', workerId, x, z, tested });
                found = true;
                return;
              }
            }
          ...
          if (!found) {
            ...
            self.postMessage({ type: 'progress', workerId, x: 0, z: 0, tested, radius: radius * 4, expanding: true });
          }
        }
        if (!found) self.postMessage({ type: 'notfound', workerId, tested });
      };
    ` : `
      class BedrockRandom {
        constructor(seed) {
      ...
      function generateBedrock(worldSeed, x, z, y, dimension) {
        ...
        for (let dy = 0; dy <= (maxY - minY); dy++) {
          ...
          positionSeed = (positionSeed + x * 2654435761) & 0xFFFFFFFF;
          positionSeed = (positionSeed + z * 1597334677) & 0xFFFFFFFF;
          positionSeed = (positionSeed + currentY * 1403630841) & 0xFFFFFFFF;
    ...
    for (let i = 0; i < 4; i++) {
      ...
      worker.onmessage = function(e) {
        ...
        if (type === 'progress') {
          ...
          if (!foundResult) {
            foundResult = true;
            setResult({ x: e.data.x, z: e.data.z, tested: e.data.tested, confidence: 100, workerId: e.data.workerId });
  ...
  const exportPattern = () => {
    ...
    const metadata = `Edition: ${edition}\nVersion: ${version}\nDimension: ${dimension}\nLayer: ${layer}\nSeed: ${seed}\n\nPattern:\n${pattern}`;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-gray-900 text-gray-100">
      {/* Navbar shared between all tabs */}
        ...
        <div className="p-4 md:p-8 pb-24">
          <div className="max-w-7xl mx-auto mb-8">
            ...
            <p className="text-gray-400 text-sm md:text-base">
              Motor de busca multi-core com precisão 100% • Suporte Java &amp; Bedrock Edition
            </p>
          </div>
                  ...
                  {[5, 7].map(size => (
                    <button
                      ...
                      className={`px-3 py-1 rounded text-sm font-medium transition-all ${
                        gridSize === size ? 'bg-purple-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
                      }`}
                    >
                      {size}×{size}
                    </button>
            ...
            <div className="space-y-6">
              <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl p-6 border border-purple-500/20 shadow-2xl">
                ...
                <div className="space-y-4">
                  {/* Edition */}
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">Edição do Minecraft</label>
                  ...
                  {/* Dimension */}
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">Dimensão</label>
                    <select
                      value={dimension}
                      onChange={(e) => {
                  ...
                  {/* Version */}
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">Versão do Minecraft</label>
                    <select
                      value={version}
                  ...
                  {/* Layer */}
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">Camada Y</label>
                    <select
                      value={layer}
                  ...
                  <div className="space-y-3">
                    {progress.workers.map((worker, idx) => (
                      <div key={idx} className="bg-gray-900/50 rounded p-3 border border-gray-700">
                        ...
                        <div className="text-xs text-gray-400 font-mono">X: {worker.x}, Z: {worker.z}</div>
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      {copied ? (
                        ...
                        <><Copy className="w-5 h-5" /> Copiar Coordenadas</>
                      )}
                    </button>
                  </div>
                </div>
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-gradient-to-t from-gray-900 to-transparent backdrop-blur-md border-t border-purple-500/20 py-4">
        <div className="max-w-7xl mx-auto px-4 text-center">
          ...
          <p className="text-xs text-gray-500 mt-1">
            Motor de busca multi-core • {activeTab === 'bedrock' ? (edition === 'java' ? 'Java Edition' : 'Bedrock Edition') : activeTab === 'ore' ? 'Ore Finder' : 'Chunk Base'}
          </p>
        </div>
      </footer>
    </div>
